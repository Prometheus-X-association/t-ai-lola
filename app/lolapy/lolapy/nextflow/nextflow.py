...
class NextflowCheckEnv():
    ...
    def check(self, docker_images_list: list[DockerImage]):
        ...
        logging.info("nextflow executable found")
        self.gen_docker_conf_file(docker_images_list)
    ...
    def gen_docker_conf_file(self, docker_images_list: list[DockerImage]):
    ...
    def run(self):
        ...
        if cmd_result.stderr:
            ...
            if error := self.__get_error_stderr(stderr=cmd_result.stderr):
                logging.error(error)
                raise error
            logging.error(cmd_result.stderr)
