...
class Health:
    ...
    def _check_database(self) -> bool:
        """Test filesystem-based dataset storage"""
        try:
            dataset_root = settings.get().lolapy_dataset_path
            # ensure folder exists
            dataset_root.mkdir(parents=True, exist_ok=True)
            
            # try creating a temp file
            test_file = dataset_root / ".healthcheck"
            test_file.write_text("ok")
            test_file.unlink()
        except Exception as e:
            logging.warning("Dataset storage healthcheck failed: {e}")
            return False
        return True
