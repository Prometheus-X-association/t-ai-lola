...
class TraxImageNotFound(CheckInstallationError):
    """Error when Trax images are not installed on the cluster server"""
    def __init__(self, image_name: str):
        if settings.get().cluster_is_backend:
            machine_message = "backend server"
        else:
            machine_message = "cluster server"
        self.message = f"Error at startup, trax image '{image_name}' is missing on {machine_message}"
        self.public_message = LolapyGlobalError.standard_public_message
